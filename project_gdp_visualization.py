    ...
    for plot_countries_code in plot_countries :
        for isp_country_code in gdpinfo :
            ...
            if gdpinfo[isp_country_code]["Country Name"] == plot_countries[plot_countries_code] : # 判断绘图库里的国家在不在世界银行数据里
                ...
                for year1 in country_imformations[4:-1] :
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    countrie1,countrie2,countrie3 = build_map_dict_by_name(gdpinfo, plot_countries, year)   # 导入清理好的数据，方便绘制到世界地图上

    worldmap_chart = pygal.maps.world.World()
    worldmap_chart = pygal.maps.world.World()
    worldmap_chart.title = '世界银行%s年国家GDP数据'%(year)    #  添加图片标题
    worldmap_chart.add('该年在绘图库及世行有数据的国家及其GDP数据', countrie1)
    worldmap_chart.add('在绘图库数据里没有的国家',countrie2)
    worldmap_chart.add('该年在世行没有GDP数据的国家',countrie3)
